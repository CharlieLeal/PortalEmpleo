...
if (isset($_GET['codProvincia'])) {
    ...
    $municipios = municipios($codProvincia);

    // Verificar si se obtuvo un resultado y devolverlo en JSON
    if ($municipios) {
        echo json_encode($municipios);
    } else {
        echo json_encode(['error' => 'No se encontraron municipios para la provincia proporcionada.']);
    }
    exit();
} else {
    echo json_encode(['error' => 'No se ha proporcionado una provincia']);
    exit();
}

function municipios($codProvincia) {
    $curl = curl_init();
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => $url . 'Provincia/' . $codProvincia . '/Municipios',
